        ...
        try (Connection conn = ConexionDB.getConexion()) {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                ...
                if (ps.executeUpdate() == 0) {
                    conn.rollback();
                    throw new SQLException("No se insertó ninguna fila al guardar la cita.");
                }
                ...
                conn.commit();
                System.out.println("[CitaRepository] Cita guardada con ID: " + cita.getId());
                return cita;
    ...
    public void actualizarEstado(int citaId, EstadoCita nuevoEstado) {
        ...
        try (Connection conn = ConexionDB.getConexion()) {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                if (ps.executeUpdate() == 0) {
                    conn.rollback();
                    throw new SQLException("Cita con ID " + citaId + " no encontrada.");
                }

                conn.commit();
                System.out.println("[CitaRepository] Estado actualizado → " + nuevoEstado);
